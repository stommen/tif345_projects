...
The left panel of Figure~\ref{fig:dalton_sim_NN} shows the corresponding results for the extended summary statistic ABC when NN-based proposals are used to generate a prior for the latent tilt $s$. The figure displays a very narrow distribution of $s$, centered about $s^*$ (offset by roughly $10^{-3}$), and a clearly more concentrated posterior for $\alpha$ around $\alpha^*$ than in Figure~\ref{fig:dalton_sim}. The middle panel shows the bead distribution of the simulated data with one standard deviation compared to the observed data. The simulated data is more sharply peaked with a lower $\pm 1\sigma$, compared to using the uniform prior in the right panel of Figure \ref{fig:dalton_sim}. It is also closer to the observed data. The right panel shows the training and validation loss for the NN used to propose the prior of $s$. The NN converges rapidly, with both training and validation loss decreasing sharply during the first few iterations and then approaching a low, stable value close to zero. The two loss curves stay very close to each other, with the validation being slightly lower throughout the iterations.
\begin{figure}[H]
    \centering
...
\section{Discussion}
The synthetic tests give a first check that the ABC method behaves in a reasonable way. For both choices of summary statistics, the posterior distributions for $\alpha$ and $s$ cover the true values $(\alpha^{*}, s^{*}) = (0.25, 0.10)$. This indicates that the method can locate the correct region in parameter space. At the same time, Table~1 shows that the quality of the inference depends strongly on both the acceptance rate and the information included in the summary.
