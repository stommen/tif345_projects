...
\end{table}
\vspace{-0.3cm}
The marginal posterior for $\Omega_{M,0}$ under flat $\Lambda$CDM (known measurement variances) is unimodal and peaks near the maximum-likelihood value reported in Table~\ref{tab_IC}, providing a consistent central estimate around $\Omega_{M,0}\approx 0.28$. The distribution is fairly narrow with an approximately symmetric core, indicating that the matter density is reasonably well constrained by the full-redshift sample in this model. The posterior mass lies well away from the boundaries $\Omega_{M,0}=0$ and $1$, and the mode and median are in close agreement with the best-fit value. Sampler traces are stationary with short autocorrelation times, indicating adequate mixing and stable estimates.
