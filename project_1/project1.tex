...
\end{align}
where $k$ is the number of parameters in the model, $N$ is the number of data points, and $\mathcal{L}^\star$ is the maximum value of the likelihood function. The model with the lower AIC and BIC scores was considered to have a better balance between goodness-of-fit and model complexity \textcolor{Maroon}{Indikerar inte högre värde för AIC/BIC bättre model?}. In this case, we assumed that the overall variance was known and equal to the sample variance of the observed distance moduli. From the maximum likelihood estimates, we also derived the matter and dark energy density parameters $\Omega_{M,0}$ and $\Omega_{\Lambda,0}$ for both models. 
...
\vspace{-0.5cm}
\section{Results}
Below we present the results obtained in the analysis of the supernova data using the Bayesian framework and MCMC sampling, as well as the model comparison between $\Lambda$CDM and wCDM cosmologies. 

Fig.~\ref{fig:corner_plot} shows a corner plot of the joint and marginal posteriors for $H_0$, $q_0$, and the variance scale $\sigma^2$, together with the corresponding trace panels. The diagonal marginals are close to Gaussian, with posterior medians and 68\% credible intervals $H_0 = 69.79^{+0.42}_{-0.41}\,\mathrm{km\,s^{-1}\,Mpc^{-1}}$, $q_0 = -0.419^{+0.068}_{-0.070}$, and $\sigma^2 = 0.968^{+0.034}_{-0.032}$. The negative $q_0$ indicates accelerated expansion, and a $\sigma^{2}$ near unity indicates that the quoted uncertainties require no overall rescaling. In the off-diagonal panels, the contours reveal a clear negative correlation between $H_0$ and $q_0$ and only weak coupling of $\sigma^2$ to the other parameters. The trace panels are stationary and show rapid mixing, consistent with adequate burn-in removal and stable sampling.
...
Using the full redshift range with $H_0=70\,\mathrm{km\,s^{-1}\,Mpc^{-1}}$ to compare the two flat background models, $\Lambda$CDM and wCDM, yields very similar maximum-likelihood estimates ($\Omega_{M,0}\approx 0.28$ and $\Omega_{\Lambda,0}\approx 0.72$), see Table~\ref{tab_IC}. The fitted $w$ is essentially $-1$, making wCDM effectively consistent with $\Lambda$CDM within the precision of this dataset.

The AIC and BIC values in Table~\ref{tab_IC} are slightly higher for $\Lambda$CDM, favoring the simpler model. This is consistent with the wCDM fit settling at $w\approx -1$ while introducing an additional parameter.
