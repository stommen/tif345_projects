...
\section{Theory}
Supernovae of type Ia are considered standardizable candles in cosmology due to their consistent intrinsic brightness. By measuring their apparent brightness $F$ and redshift $z$, we can infer distances and thus probe the expansion history of the universe. The distance modulus $\mu$ relates the apparent magnitude $m$ and absolute magnitude $M$ of a supernova as follows:
...
\end{equation}
where $\Omega_M$, $\Omega_k$, and $\Omega_\Lambda$ are the matter, curvature, and cosmological constant (dark energy) density parameters respectively. By normalizing the paramaters with present values (denoted by subscript '0'), one can get the reduced version of the so called $\Lambda$CDM cosmology
\begin{equation}
...
In addition to the posterior analysis, posterior predictive checks were performed to assess the model fit. Using the posterior samples, predicted distance moduli $\mu_{\text{model}}$ were generated across a range of redshift values and compared to the observed data. The median and credible intervals of the predicted distance moduli were plotted alongside the observed data to visually evaluate the model's performance.

The second part of the project focused on comparing the flat $\Lambda$CDM and wCDM cosmological models.  

\section{Results}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\textwidth]{figs/post_samples_H0_q0_sigma2.png}
    \caption{Corner plot showing the posterior distributions and correlations between the parameters $H_0$, $q_0$, and $\sigma_{\mathrm{sys}}^2$. The histograms on the diagonal represent the marginal distributions for each parameter, while the off-diagonal plots show the joint distributions.}
    \label{fig:corner_plot}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figs/mu_model_pred.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figs/Om_marginal_posterior.pdf}
    \caption{Marginal posterior distribution of the matter density parameter $\Omega_{M,0}$ under the wCDM model.}
